...
const Dashboard = ({setTabSelected }) => {
    ...
    const { username, profilePicture, status ,timezone , availability } = useSelector((state) => state.user);
    ...
    return (
        <div className="Dashboard-Wrapper w-full h-full bg--300">
            <h1 className="text-5xl text-blue-500 font-bold">Dashboard</h1>
            <p className="text-lg text-zinc-500 font-medium mt-4">Welcome to your Dashboard! Manage your profile, update availability, and stay organized—all in one place</p>
            <div className="w-full bg--300 flex  gap-10 py-20">
                 <div className="Profile-Card  size-[275px] flex justify-end pt-[100px]">
                     <div className="relative bg-blue-500 rounded-xl w-full h-full flex flex-col gap-4 justify-center items-start px-2">
                 ...
                 <div className="Availablity-User bg--400  h-fit grid grid-cols-3 gap-6">
                 {Object.entries(availability).map(([day, { isAvailable, startTime, endTime }]) => (
                  ...
                  <button onClick={() => setTabSelected("Availability")} className="bg-[#006BFF] text-white rounded-md font-medium">Availability</button>
                 </div>
            </div>
        </div>
    );
